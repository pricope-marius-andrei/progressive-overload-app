...
const WorkoutIdLayout: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center bg-indigo-50 dark:bg-indigo-950 px-6">
  ...
  if (isValidWorkout === null) {
    return (
      <View className="flex-1 items-center justify-center bg-indigo-50 dark:bg-indigo-950 px-6">
        <View className="w-full max-w-sm items-center rounded-2xl border border-white/70 dark:border-indigo-900/60 bg-white/70 dark:bg-slate-900/70 p-6">
          <ActivityIndicator size="small" color={COLORS.primary} />
          <Text className="mt-3 text-base text-indigo-700 dark:text-indigo-200">
            Loading workout...
          </Text>
        </View>
      </View>
  ...
  if (!workoutId || !isValidWorkout) {
    return (
      <View className="flex-1 items-center justify-center bg-indigo-50 dark:bg-indigo-950 px-6">
        <View className="w-full max-w-sm items-center rounded-2xl border border-white/70 dark:border-indigo-900/60 bg-white/70 dark:bg-slate-900/70 p-6">
          <Text className="mb-1 text-base font-semibold text-indigo-900 dark:text-indigo-50">
            Workout not found
          </Text>
          <Text className="text-center text-sm text-indigo-700 dark:text-indigo-200">
